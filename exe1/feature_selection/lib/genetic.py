...
class FeatureSelectionGA:
	...
	def __init__(self, model, x, y, cv_split=5, verbose=0):
		...
		if self.verbose == 1:
			print("Model {} will select best features among {} features using cv_split :{}.".format(
				model, x.shape[1], cv_split))
	...
	def _init_toolbox(self):
		...
		toolbox.register("individual", tools.initRepeat,
						 creator.Individual, toolbox.attr_bool, self.n_features)
	...
	def generate(self, n_pop, cxpb=0.5, mutxpb=0.2, ngen=5, set_toolbox=False):
		...
		if self.verbose == 1:
			print("Population: {}, crossover_probablity: {}, mutation_probablity: {}, total generations: {}".format(
		...
		for g in range(NGEN):
			...
			self.fitness_in_generation[str(
				g+1)] = max([ind.fitness.values[0] for ind in pop])
