...
for i=1:3
    ...
    lambda = 0.1;              %% lambda for the regularization
	dim = 100;                    
	kernel_type = 'linear';   
	gamma = 0;               
	T = 10; 
    [Acc(i),~,~] = TJM_(Xs,Ys,Xt,Yt,options);
    fprintf('type %d: %.2f accuracy \n\n', i, Acc(i) * 100);
end
if ~exist('results/TJM/','dir')
    mkdir('results/TJM/');
end
save('results/TJM/TJM.txt','Acc','-ascii');

function [acc,acc_list,A] = TJM_(X_src,Y_src,X_tar,Y_tar,options)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    svm_model = fitcecoc(Zs,Y_src,'Learners',template); 
    Y_tar_pseudo = svm_model.predict(Zt');
